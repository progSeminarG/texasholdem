...
class Dealer(object):
    ...
    def get_response(self):
        ...
        self.flag = 0
        while self.flag<len(self.__players) and len(self.active_plyers_list)!= 1: # while 文で flag がプレイヤー数になるという次の工程に移行する条件を定義
            self.resplist = []
            self.active_plyers_list = []
            self.__resp = [
                ...
                if self.flag>= len(self.__players):
                    ...
                    if self.playercheck[i] == 1: # レイズしてきたプレーヤが本当にさんかしつづけているか？
                        if self.minimum_bet>self.resplist[i]: # minimum_bet より小さい金額なら minimum_bet に修正
                            ...
                            self.minimum_bet = self.resplist[i] # minimum_bet の更新
                        self.money = self.money+self.resplist[i] # call 金額の更新
                        self.flag = 1
                        self.bettingrate[i] = self.money
                    else:
                        self.flag = self.flag+1
